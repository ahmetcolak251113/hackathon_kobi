...
def create_shipment(db: Session, company: Company, payload: ShipmentCreate) -> ShipmentCreateResponse:
...
def list_company_shipments(
    ...
    query = db.query(Shipment).filter(Shipment.company_id == company.id)
    if status:
