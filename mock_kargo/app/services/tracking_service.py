...
from app.schemas.shipment import ShipmentTrackingResponse, ShipmentItemInTrackingResponse
...
def get_tracking_response(db: Session, tracking_code: str) -> ShipmentTrackingResponse:
    ...
    for shipment_item in shipment.items:
        product_name = (
            shipment_item.product.name 
            if shipment_item.product_id and shipment_item.product 
            else shipment_item.custom_product_name or "Bilinmeyen Ürün"
        )
        items.append(
            ShipmentItemInTrackingResponse(
